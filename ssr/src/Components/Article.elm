module Components.Article exposing (article)

import Css
import Css.Global
import Html.Styled exposing (div, Html)
import Html.Styled.Attributes exposing (css)

import Core.Theme exposing (Theme)
import Utils.Markdown.Markdown exposing (parseMarkdown)

type alias Props message =
  { theme: Theme
  , content: String
  , meta: Html message
  , header: Html message
  }

content : Props message -> Html message
content props =
  div
    [ css
        [ Css.marginBottom <| Css.px 15
        , Css.Global.children
            [ Css.Global.typeSelector "p a, ul a" [ Css.color props.theme.accent ]
            , Css.Global.typeSelector "pre" [ Css.width <| Css.pct 100 ]
            , Css.Global.typeSelector "p strong, blockquote p strong" [ Css.color props.theme.textPrimary ]
            , Css.Global.typeSelector "blockquote"
              [ Css.color props.theme.textPrimary
              , Css.paddingLeft (Css.px <| 10)
              , Css.borderLeft3 (Css.px <| 2) Css.solid props.theme.accent
              ]
            , Css.Global.typeSelector "p code, ul code"
              [ Css.color props.theme.codeColor
              , Css.backgroundColor props.theme.codeBackground
              , Css.padding2 Css.zero (Css.px <| 5)
              ]
            , Css.Global.typeSelector "pre code"
              [ Css.color props.theme.codeColor
              , Css.backgroundColor props.theme.codeBackground
              , Css.padding <| Css.px 15
              , Css.width <| Css.pct 100
              ]
            , Css.Global.typeSelector "p img"
              [ Css.margin2 Css.zero Css.auto
              , Css.maxWidth <| Css.pct 100
              , Css.height Css.auto
              ]
            , Css.Global.typeSelector "p center"
              [ Css.color props.theme.textPrimary
              , Css.fontSize <| Css.rem 0.8
              ]
            ]
        ]
    ]
    (parseMarkdown props.content)

article : Props message -> Html message
article props =
  div
    [ css
        [ Css.maxWidth <| Css.px 750
        , Css.margin2 Css.zero Css.auto
        ]
    ]
    [ props.header, content props, props.meta ]
